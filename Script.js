...
window.onload = function () {
	...
	//クリック音読み込み
	var ClickSound = "burin.wav";						//game.htmlからの相対パス
	game.preload([ClickSound]); 				//データを読み込んでおく
	
	//終了時サウンド（ボクヌマタ）
	var BNSound = "BN.wav";
	game.preload([BNSound]);

	//ぞう山くん画像
	var ZoyamaImg = "Numata.png";						//game.htmlからの相対パス
	game.preload([ZoyamaImg]);					//データを読み込んでおく

	//沼田画像(終了時)
	var BN = "BokuNumata.png";
	game.preload([BN]);
	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		S_MAIN.backgroundColor = "pink"; 			//S_MAINシーンの背景は黒くした
		...
		//ぞう山ボタン
		var Zoyama = new Sprite(100, 178);				//画像サイズをここに書く。使う予定の画像サイズはプロパティで見ておくこと
		...
		game.onenterframe = function () {
			...
			if (Zoyama.x >= 400) {						//画面端にぞう山画像が行ってしまったら
				...
				S_BakaText.text = "バーカww";       
				
				if(BN_flag){
					//終了時サウンド＝ボクヌマタ
					game.assets[BNSound].play();
		...
		//僕沼田
		var S_Numata = new Sprite(400,500);
		S_Numata.moveTo(0,0);
		S_Numata.image = game.assets[BN];
		S_END.addChild(S_Numata);

		//GAMEOVER
		var S_GameOverText = new Label(); 					//テキストはLabelクラス
		S_GameOverText.font = "20px Meiryo";				//フォントはメイリオ 20px 変えたかったらググってくれ
		S_GameOverText.color = "white";                     //色：今回は白
		...
		//バーカww
		var S_BakaText = new Label();
		S_BakaText.font = "85px Meiryo";
		S_BakaText.color = "white";
		S_BakaText.width = 400;
		S_BakaText.moveTo(0,100);
		S_END.addChild(S_BakaText);
		...
		S_Tweet.moveTo(230, 400);						//ツイートボタンの位置
